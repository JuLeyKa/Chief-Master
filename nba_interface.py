import os 
# Setze den Server-Port auf 8502 (alternative Portwahl)
...
st.markdown(
    ...
    /* Banner-Bild (CSS-Klasse) */
    .banner-img {
        width: 100%;
        max-height: 80px; /* hier kannst du die gewünschte Höhe anpassen */
        object-fit: cover;
        display: block;
        margin: 0 auto;
    }
...
if os.path.exists(banner_path):
    ...
    st.markdown(
        f"<img class='banner-img' src='file:///{banner_path_forward}'>",
        unsafe_allow_html=True
    )
else:
    st.warning("Bild konnte nicht geladen werden. Bitte Pfad prüfen.")

st.markdown("<h1 class='title'>🏀 NBA Chief – by Master Halil</h1>", unsafe_allow_html=True)
...
# -----------------------------------
# Hilfsfunktionen (Spiel-/Statistikabfragen)
# -----------------------------------
...
# -----------------------------------
# Zusätzliche Hilfsfunktionen für Visualisierungen
# -----------------------------------
...
def get_combined_boxscores(loaded_data):
    """
    Fasst Home- und Away-Boxscores zusammen und ordnet anhand der TEAM_ID den Spielern
    das richtige Team zu. Dafür werden die in den gespeicherten Daten mitgegebenen
    "home_team_id" und "away_team_id" genutzt.
    """
    all_records = []
...
def get_boxscore_for_game(game_id):
    ...
    # Umbenennen der Spalte PLAYER_NAME in PLAYER, falls vorhanden
...
# -----------------------------------
# Erweiterte Visualisierungen (nur bei geladenen historischen Begegnungen)
# -----------------------------------
...
def plot_stats_correlation(loaded_data):
    ...
    fig = px.imshow(
        corr_matrix,
        text_auto=True,
        title="Korrelation zwischen Spieler-Statistiken"
    )
    st.plotly_chart(fig)

def plot_team_radar(loaded_data):
    df_all = get_combined_boxscores(loaded_data)
    if df_all.empty:
    ...
    for idx, row in df_team.iterrows():
        ...
        fig.update_layout(
            polar=dict(
                radialaxis=dict(
                    visible=True,
                    range=[0, df_team[stats_cols].max().max() + 5]
                )
            ),
            showlegend=False,
...
# Neue Hilfsfunktionen für die Anzeige im geladenen Modus

def compute_team_outcome(perf_df):
    """Berechnet für ein Team aus der Spielerleistungs-DataFrame
    die prognostizierten Gesamtsummen für PTS, REB, AST, STL und BLK."""
    if perf_df.empty:
        return {}
    outcome = {
...
def plot_aggregated_player_stats_for_team(perf_df, team_label):
    """Erstellt ein Balkendiagramm der durchschnittlichen Spielerstatistiken (letzten 5 Spiele)
    für die angegebene Mannschaft."""
    if perf_df.empty:
...
def plot_team_radar_enhanced(loaded_data):
    """Zeigt größere, buntere Team-Radar-Diagramme an."""
    df_all = get_combined_boxscores(loaded_data)
    if df_all.empty:
        st.warning("Keine Boxscore-Daten für Team-Radar gefunden.")
        return
    ...
    for idx, row in df_team.iterrows():
        ...
        fig.update_layout(
            polar=dict(
                radialaxis=dict(
                    visible=True,
                    range=[0, df_team[stats_cols].max().max() + 10]
                )
            ),
            showlegend=True,
            title=f"Team-Radar – {row['TEAM']}",
...
if os.path.exists(history_csv):
    ...
    history_df["label"] = history_df.apply(
        lambda row: f"{row['timestamp']} | {row['home_team']} vs {row['away_team']} | {float(row['prob']):.0%}", axis=1
    )
    selected_history = st.selectbox(
        "▶ Vergangene Begegnung laden",
        ["(neu)"] + history_df["label"].tolist(),
        key="history_select"
    )
else:
...
if selected_history != "(neu)":
    ...
    if os.path.exists(json_path):
        ...
        # Gespeicherte Spielerleistungs-Vorhersagen anzeigen
        ...
        # NEU: Berechnung und Anzeige der prognostizierten Spielausgangstabelle
        if "player_performance_home" in loaded_data and "player_performance_away" in loaded_data:
            ...
            # NEU: Aggregierte Spielerstatistiken (Balkendiagramme) für beide Teams
            ...
            # NEU: Größere Darstellung der Korrelation zwischen Spieler-Statistiken
            ...
            # NEU: Größere, farbenfrohere Team-Radar-Diagramme
            st.markdown("### Team-Radar-Diagramme (Groß & Bunt)")
            plot_team_radar_enhanced(loaded_data)

        # Zum Schluss: Anzeige der gespeicherten Boxscore-Tabellen
...
# Eindeutige Keys für die selectboxen
home_team = st.selectbox("Home Team", team_names, index=team_names.index(home_team), key="home_team_select")
away_team = st.selectbox("Away Team", team_names, index=team_names.index(away_team), key="away_team_select")
...
if st.button("Vorhersage starten", key="btn_predict"):
    ...
    # -------------------------------
    # NEUER ABSCHNITT: Vorhersage der Spielerleistung
    # -------------------------------
    st.markdown("### Vorhersage der Spielerleistung für die nächste Begegnung")
    
    # Boxscores für die letzten 5 Spiele beider Teams abrufen
    ...
    # Hilfsfunktion zur Berechnung der Spielerleistung,
    # wobei nur Spieler berücksichtigt werden, deren TEAM_ID mit der ausgewählten Mannschaft übereinstimmt
    ...
    # JSON speichern (zusätzlich speichern wir nun die Team-IDs sowie die Spielerleistungs-Vorhersagen)
    big_dict = {
        "home_team": home_team,
        "away_team": away_team,
        "home_team_id": team_id_map[home_team],
